...
def main():
    ...
    lsvm_cv_scores1 = cross_validate(
        lsvm_grid1, features, class1, cv=cv, return_estimator=False, scoring="f1_macro"
    )

    lsvm_cv_scores2 = cross_validate(
        lsvm_grid2, features, class2, cv=cv, return_estimator=False, scoring="f1_macro"
    )

    cv_dict["LSVM"] = {
        "year": np.mean(lsvm_cv_scores1["test_score"]),
    ...
    lr_pipe = Pipeline(steps=[("vect", vectorizer), ("clf", LogisticRegression())])
    ...
    cv_dict["LR"] = {
        "year": np.mean(lr_cv_scores1["test_score"]),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="mlclf", description="Comparison of LSVM and LR classification."
    )
    parser.add_argument(
        "--corpus_name",
        "-cn",
        type=str,
        default="year",
        help="Indicates the corpus. Default is 'year'. Another possible value is 'poet'.",
    )
    parser.add_argument(
        "--lowercase",
        "-l",
